 ...
 *   └── Security
 *         ├── Password         change (email/password accounts)
 *         ├── Cloud PIN        change — verified and stored by Firebase only
 *         ├── App Lock PIN     this device only — never synced
 *         └── Security questions  App Lock recovery — this device only
 *
 * Also hosts the two mandatory flows the sign-in gate can trigger:
 * forcing a password change and forcing a Cloud PIN change.
 */

import {
  getCurrentUser, getCurrentUserData, isSuperAdmin, ROLES, ROLE_BADGE,
  formatFirebaseError, changeMyPassword, doSignOut,
} from './auth.js';
import {
  changeCloudPin, finishPasswordSetup, getMyPinStatus,
} from './staff-auth.js';
import {
  DEFAULT_SECURITY, validateCloudPinPolicy, validatePasswordPolicy,
...
function passwordInputHTML({ id, label, autocomplete = 'new-password', hint = '' }) {
  return `<div class="field"><label for="${id}">${label}</label><div class="input-affix">
...
export async function openProfileModal({ stations = [], onSignOut } = {}) {
  ...
  const hasEmail = !!(userData.email || getCurrentUser()?.email);
  ...
  byId('profile-modal-body').innerHTML = `
    ...
    <dl class="profile-settings-list profile-detail-grid">
      <dt>Username</dt><dd>@${h(userData.username || '—')}</dd>
      <dt>Email</dt><dd>${h(userData.email || '—')}</dd>
      <dt>Phone</dt><dd>${h(userData.phoneNumber || '—')}</dd>
      <dt>Employee ID</dt><dd>${h(userData.employeeId || '—')}</dd>
    ...
    <section class="profile-security" aria-labelledby="profile-security-title">
      ...
      <div class="security-row">
        <div class="security-row-text">
          ...
          <small>${hasEmail ? 'Used with your email or username to sign in.' : 'This account has no password. Contact your admin to add one.'}</small>
        </div>
        <button type="button" id="profile-change-password" class="btn btn-secondary btn-small" ${hasEmail ? '' : 'disabled'}>${ICONS.key} Change</button>
      </div>

      <div class="security-row">
        <div class="security-row-text">
          <strong>Cloud PIN</strong>
          <small>Verified by Firebase and stored securely — never in this app.</small>
        </div>
  ...
  byId('profile-change-password')?.addEventListener('click', () => openChangePasswordForm());
...
export async function openChangePinForm() {
  ...
  showFormModal('Change Cloud PIN', `<form id="change-pin-form" novalidate>
    <p class="modal-intro">Your Cloud PIN is verified by Firebase. It is never saved in this app or synced to this device.</p>
...
// ═══════════════════════════════════════════════════════════════════════
//  Password — change (Profile → Security)
// ═══════════════════════════════════════════════════════════════════════
export async function openChangePasswordForm() {
  const status = await getMyPinStatus().catch(() => null);
  const policy = status?.policies || DEFAULT_SECURITY;
  showFormModal('Change Password', `<form id="change-password-form" novalidate>
    <p class="modal-intro">Passwords are authenticated by Firebase Authentication. Your new password applies to future sign-ins immediately.</p>
    ${passwordInputHTML({ id: 'current-password', label: 'Current password', autocomplete: 'current-password' })}
    ${passwordInputHTML({ id: 'new-password', label: 'New password', hint: passwordHint(policy) })}
    ${passwordInputHTML({ id: 'confirm-password', label: 'Confirm new password' })}
    <p id="change-password-error" class="form-error hidden" role="alert"></p>
    <button type="submit" class="btn btn-primary btn-full">Save Password ${ICONS.save}</button>
  </form>`);
  ...
  byId('change-password-form').addEventListener('submit', async event => {
    ...
    if (!currentPassword) return fieldFail('change-password-error', '❌ Enter your current password.');
    const invalid = validatePasswordPolicy(newPassword, policy);
    if (invalid) return fieldFail('change-password-error', invalid);
    if (newPassword !== confirm) return fieldFail('change-password-error', '❌ New passwords do not match.');
    setBusy(button, true, 'Saving…');
    try {
      await changeMyPassword({ currentPassword, newPassword });
      await finishPasswordSetup().catch(() => {});
      closeModal('generic-modal');
      toastSuccess('Password Updated');
    } catch (err) {
      fieldFail('change-password-error', `❌ ${formatFirebaseError(err)}`);
      setBusy(button, false);
    }
  });
}

function passwordHint(policy) {
  const parts = [`At least ${policy.minPasswordLength || 8} characters`];
  if (policy.passwordComplexity === 'lettersNumbers') parts.push('with letters and numbers');
  if (policy.passwordComplexity === 'strong') parts.push('with uppercase, lowercase, a number, and a symbol');
  return `${parts.join(' ')}.`;
}
...
// ═══════════════════════════════════════════════════════════════════════
export function openForcedPasswordChange(status) {
  return new Promise(resolve => {
    const policy = status?.policies || DEFAULT_SECURITY;
    showFormModal('Create your new password', `<form id="forced-password-form" novalidate>
      <p class="modal-intro">Your account was issued a temporary password. Create your own password to continue.</p>
      ${passwordInputHTML({ id: 'forced-current-password', label: 'Temporary password', autocomplete: 'current-password' })}
      ${passwordInputHTML({ id: 'forced-new-password', label: 'New password', hint: passwordHint(policy) })}
      ${passwordInputHTML({ id: 'forced-confirm-password', label: 'Confirm new password' })}
      <p id="forced-password-error" class="form-error hidden" role="alert"></p>
      <button type="submit" class="btn btn-primary btn-full">Save Password ${ICONS.save}</button>
      <button type="button" class="link-btn forced-signout">Sign out instead</button>
    </form>`, { locked: true });

    byId('forced-password-form').addEventListener('submit', async event => {
      event.preventDefault();
      const button = event.currentTarget.querySelector('button[type="submit"]');
      const currentPassword = byId('forced-current-password').value;
      const newPassword = byId('forced-new-password').value;
      const confirm = byId('forced-confirm-password').value;
      if (!currentPassword) return fieldFail('forced-password-error', '❌ Enter the temporary password.');
      const invalid = validatePasswordPolicy(newPassword, policy);
      if (invalid) return fieldFail('forced-password-error', invalid);
      if (newPassword !== confirm) return fieldFail('forced-password-error', '❌ New passwords do not match.');
      setBusy(button, true, 'Saving…');
      try {
        await changeMyPassword({ currentPassword, newPassword });
        await finishPasswordSetup().catch(() => {});
        ...
        fieldFail('forced-password-error', `❌ ${formatFirebaseError(err)}`);
        setBusy(button, false);
      }
    });
    byId('forced-password-form').querySelector('.forced-signout').addEventListener('click', async () => {
      setModalLocked('generic-modal', false);
      closeModal('generic-modal');
      await doSignOut().catch(() => {});
      resolve();
  ...
  return new Promise(resolve => {
    ...
    showFormModal(rotating ? 'Cloud PIN rotation required' : 'Set your Cloud PIN', `<form id="forced-pin-form" novalidate>
      <p class="modal-intro">${rotating
        ...
        : 'Your account was issued a temporary Cloud PIN. Create your own to continue.'} The PIN takes effect for future sign-ins immediately.</p>
