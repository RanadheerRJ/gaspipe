 ...
 * controls which sign-in methods appear on the login screen, whether the
 * local App Lock is required, and the password / Cloud PIN policies applied
 * to accounts assigned to that station.
 *
 * The document contains only feature flags and policy numbers — never
 * secrets — so Firestore rules allow unauthenticated reads. The login screen
 * needs the flags BEFORE a user signs in; writes remain manager-only and the
 * Cloud Functions re-check them on every PIN sign-in, so the UI can never
 * weaken server-side enforcement.
 */
...
export const DEFAULT_SECURITY = Object.freeze({
  // Sign-in methods — each combination of identifier × secret is a method:
  //   email+password, username+password, email+Cloud PIN, username+Cloud PIN
...
export const LOGIN_METHODS = Object.freeze([
  ...
  { id: 'email-password', identifier: 'email', secret: 'password', label: 'Email + Password', icon: '📧' },
  { id: 'username-password', identifier: 'username', secret: 'password', label: 'Username + Password', icon: '@' },
]);
